...
from torch_geometric.nn import GCNConv, GATConv, EdgeConv, SAGPooling
import math
...
class GDN(nn.Module):
    ...
    def forward(self, data, org_edge_index):
        ...
        graph = []
        for i, edge_index in enumerate(edge_index_sets): # edge_index is a single dimensional tensor with the first index being the parent edges and the second index being the child edges
            edge_num = edge_index.shape[1] # total number of edges
            ...
            #np_corr_mat = np_corr_mat[0:rows, 0:rows]
            #corr_ji_mat = torch.Tensor(np_corr_mat)
            ...
            topk_num = self.topk
            #print(cos_ji_mat)
            #print(cos_ji_mat.shape)
            #topk_indices_ji = torch.topk(corr_ji_mat, topk_num, dim=-1)[1]
